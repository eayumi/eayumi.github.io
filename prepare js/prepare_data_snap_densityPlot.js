...
function prepare_snap_densityPlot() {
    ...
    d3.json('../data/data.json').then(function(datas) {
        ...
        sensname = { 'Drehwand': 0, 'Drehschrank': 1, 'LampeAussenwand': 2, 'LampeDrehwand': 3 };
        ...
        for (var j = 0; j < 4; j++) {
            ...
            if (day(first) != day(second)) {
                ...
                datanew[j].push({
                    ...
                    split: 1 //indicates was the first part in the split
                ...
                datanew[j].push({
                    ...
                    To: data[j][0].To,
                    Timestamp: temp,
                    split: 2 //indicates was the second part in the split
        ...
        snap_data.push({
            ...
            element_ang: [dw_angle, ds_angle, la_angle, ld_angle],
            time_to_next: 0,
            ...
            Hour: new Date("2019-09-11 20:43:46.000").getHours(),
            Min: new Date("2019-09-11 20:43:46.000").getMinutes()
        ...
        var addms = { 'Mon': 0, 'Tue': 1, 'Wed': 2, 'Thu': 3, 'Fri': 4, 'Sat': 5, 'Sun': 6 }


        function startOfWeek(date) {
            date = new Date(date)

            var diff = date.getDate() - addms[weekday(date)]
            console.log(date);
            d = day(new Date(date.setDate(diff)));
            console.log(d);
            if (date == new Date(date.setDate(diff))) return -1;
            return d;
        ...
        for (var i = 0; i < snap_data.length - 1; i++) {
            ...
            snap_clean.push(snap_data[i])

            if (snap_data[i].Week != snap_data[i + 1].Week) {

                console.log(snap_data[i + 1].Week)
                dt = (startOfWeek(snap_data[i + 1].Timestamp));
                if (dt != -1) {
                    t = new Date(dt + ' 00:00:00.000');
                    console.log(t)
                        // snap_data[i].time_to_next = t - new Date(snap_data[i].Timestamp);

                    newsnap = {
                        Timestamp: timej(t),
                        element_ang: snap_data[i].element_ang,
                        time_to_next: 0,
                        Week: week(t),
                        Weekday: weekday(t),
                        Group: (weeks_single.includes(week(t)) ? 0 : (!weeks_out.includes(t)) ? 1 : -1),
                        Hour: t.getHours(),
                        Min: t.getMinutes(),
                        Element: snap_data[i].Element,
                        split: -1
                    }

                    snap_clean.push(newsnap)
                }
            }

        }
        snap_clean.push(snap_data[snap_data.length - 1])


        for (var i = 0; i < snap_clean.length - 1; i++) {

            snap_clean[i].time_to_next = new Date(snap_clean[i + 1].Timestamp) - new Date(snap_clean[i].Timestamp);
            snap_clean[i].time_to_next = (snap_clean[i].time_to_next < 0) ? 0 : snap_clean[i].time_to_next;
        }
        snap_clean.filter(d => !weeks_out.includes(d.Week))

        console.log(snap_clean)
        toJson(snap_clean, 'data_snaps_densityPlot.json')
    })
...
function prepare_snap_densityPlot_Besucher() {
    ...
    d3.json('../data/data_Besucher.json').then(function(datas) {
        ...
        console.log(snap_clean)
        toJson(snap_clean, 'data_snaps_densityPlot_Besucher.json')
    })
