...
(function() {
    d3v3411.parsets = function() {
        var event = d3v3411.dispatch("sortDimensions", "sortCategories"),
        ...
        function parsets(selection) {
            selection.each(function(data, i) {
                var g = d3v3411.select(this),
                    ordinal = d3v3411.scale.ordinal(),
                    dragging = false,
                ...
                d3v3411.select(window).on("mousemove.parsets." + ++parsetsId, unhighlight);
                ...
                updateDimensions();
                if (tension != tension0) {
                    var t = d3v3411.transition(g);
                    if (t.tween) t.tween("ribbon", tensionTween);
                    else tensionTween()(1);
                }

                function tensionTween() {
                    var i = d3v3411.interpolateNumber(tension0, tension);
                    return function(t) {
                        tension0 = i(t);
                ...
                function updateDimensions() {
                    ...
                    ordinal.domain([]).range(d3v3411.range(dimensions[0].categories.length));
                    ...
                    dimension
                        .call(d3v3411.behavior.drag()
                            ...
                            .on("dragend", function(d) {
                                ...
                                transition(d3v3411.select(this))
                                    .attr("transform", "translate(0," + d.y + ")")
                                    .tween("ribbon", ribbonTweenY);
                            }));
                ...
                function sortBy(type, f, dimension) {
                    return function(d) {
                        ...
                        d3v3411.select(this).text(direction > 0 ? type + " \u2192" : "\u2190 " + type); //modified
                ...
                function ribbonTweenX(d) {
                    var nodes = [d],
                        ...
                        i = nodes.map(function(d) { return d3v3411.interpolateNumber(d.x0, d.x); }),
                        n = nodes.length;
                    return function(t) {
                ...
                function ribbonTweenY(d) {
                    var r = ribbon.filter(function(r) { return r.source.dimension.name == d.name || r.target.dimension.name == d.name; }),
                        i = d3v3411.interpolateNumber(d.y0, d.y);
                    return function(t) {
                        d.y0 = i(t);
                        r.attr("d", ribbonPath);
                    };
        ...
        var body = d3v3411.select("body");
        var tooltip = body.append("div")
            .style("display", "none")
            .attr("class", "parsetstooltip");

        return d3v3411.rebind(parsets, event, "on").value(1).width(960).height(600);
    ...
    d3v3411.parsets.tree = buildTree;

    function autoDimensions(d) {
        return d.length ? d3v3411.keys(d[0]).sort() : [];
    }

    function cancelEvent() {
        d3v3411.event.stopPropagation();
        d3v3411.event.preventDefault();
    }
    ...
    var percent = d3v3411.format("%"),
        comma = d3v3411.format(",f"),
        parsetsEase = "elastic",
        parsetsId = 0;

    // Construct tree of all category counts for a given ordered list of
    // dimensions.  Similar to d3v3411.nest, except we also set the parent.
